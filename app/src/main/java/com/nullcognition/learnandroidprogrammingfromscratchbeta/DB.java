...
/**
 * Created by ersin on 01/10/14 at 3:00 PM
 */
public class DB extends android.database.sqlite.SQLiteOpenHelper {
  ...
  DB(android.content.Context context, String name, android.database.sqlite.SQLiteDatabase.CursorFactory factory) {
	super(context, name, factory, DB_VERSION);
  }

  @Override
  public void onCreate(android.database.sqlite.SQLiteDatabase dbCountries) {
	createTable(dbCountries);
  }

  @Override
  public void onUpgrade(android.database.sqlite.SQLiteDatabase dbCountries, int oldVersion, int newVersion) {
	String dropSQL = "DROP TABLE IF EXISTS " + COUNTRY_TABLE_NAME + ";";
	dbCountries.execSQL(dropSQL);
	createTable(dbCountries);
  }

  private void createTable(android.database.sqlite.SQLiteDatabase dbCountries) {
